...
userRoutes.post("/create", upload.single("file"), async (req, res) => {
    ...
    const createUsersController = new CreateUsersController(createUsersRepository, cryptographyPassword);
    if (req.file) {
...
userRoutes.patch("/update/:id", authenticateToken.authenticateToken, upload.single("file"), async (req, res) => {
    const updateUsersRepository = new UpdateUsersRepository();
    const updateUsersController = new UpdateUsersController(updateUsersRepository);
    req.body.profile_photo = `http://localhost:3000/files/${req.file.filename}`;
...
userRoutes.delete("/delete/:id", authenticateToken.authenticateToken, async (req, res) => {
    const deleteUsersRepository = new DeleteUsersRepository();
    const deleteUsersController = new DeleteUsersController(deleteUsersRepository);
...
userRoutes.get("/findUsername", async (req, res) => {
    ...
    const getUsersByUsernameController = new GetUsersByUsernameController(getUsersByUsernameRepository);
...
userRoutes.patch("/addFollower", async (req, res) => {
    ...
    const addFollowerController = new AddFollowerController(addFollowerRepository);
...
userRoutes.patch("/deleteFollower", async (req, res) => {
    ...
    const deleteFollowerController = new DeleteFollowerController(deleteFollowerRepository);
