...
commentRoutes.get("/", async (req, res) => {
    ...
    const getCommentsController = new GetCommentsController(getCommentsRepository);
    const { body, statusCode } = await getCommentsController.handle();
    res.status(statusCode).send(body);
});
commentRoutes.post("/create", authenticateToken.authenticateToken, async (req, res) => {
    const createCommentsRepository = new CreateCommentsRepository();
    const createCommentsController = new CreateCommentsController(createCommentsRepository);
    const { body, statusCode } = await createCommentsController.handle({
        body: req.body,
    });
    res.status(statusCode).send(body);
});
commentRoutes.delete("/delete/:id", authenticateToken.authenticateToken, async (req, res) => {
    const deleteCommentsRepository = new DeleteCommentsRepository();
    const deleteCommentsController = new DeleteCommentsController(deleteCommentsRepository);
    const { body, statusCode } = await deleteCommentsController.handle({
        params: req.params,
    });
    res.status(statusCode).send(body);
});
commentRoutes.get("/myComments", authenticateToken.authenticateToken, async (req, res) => {
